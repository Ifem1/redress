...
The frontend connects to an injected wallet (e.g. MetaMask) and prompts to add/switch to GenLayer StudioNet (chain id `61999` / `0xF22F`) using standard `wallet_switchEthereumChain` / `wallet_addEthereumChain` calls — no browser extension beyond a standard EVM wallet is required. The current verified deployment for source commit `e4ff9f4` is `0x747F3f8B86948676E5A50adD72790ed00878Dd5A`; deployment transaction `0xd016a8d9dedefbdf371082f746f9123f6271ceff1711a8143dbc988417da8f28` reached `ACCEPTED` with successful execution and majority agreement.
...
The prior deployment at `0xd7BbbbC1c7DcbD44382a53472445833734533727` was funded live through GenLayerJS; the current deployment requires a fresh payable proof.
...
A third funded live case used `VENUE-3` with the public Redress README as its governing policy: funding transaction `0xac9fbd3fed13824267c90fd2a96a5a4803080195bbdd5a93b6aed056c0cbe81f`, complaint transaction `0x50b04ff2903c20465a30a6d6ccf95528281d615872db5b90511aacccb326b9b2`, evidence-lock transaction `0x114bf3e8e879b27ec43dc731fc1bd21e88190f49786b7275a5f7f4c2e4a46439`, and review transaction `0x8da70aa5ccf64aa63fe41a93eea6dcdd5aa7cede6ed796020fa2c9883c2afbf4` all executed on the final deployment. Validators independently retrieved the real public policy, stored its digest and compact manifest, and correctly returned `dismissed_insufficient_evidence` with `approved_amount=0`; canonical accounting remained `pool_balance=1 GEN`, `pool_reserved=0`, and `pool_paid=0`. Future versions may replace manifest `content_length` comparison with more stable derived evidence fields for highly dynamic pages.
